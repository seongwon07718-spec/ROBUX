import { Client, GatewayIntentBits, Partials, InteractionType, ModalBuilder, TextInputBuilder, TextInputStyle, ActionRowBuilder } from "discord.js";
...
async function validateChannel(channelId) {
  try {
    ...
    // 0: GuildText (discord.js v14에서 ch.type === 0 이면 텍스트 채널)
    if (!ch || ch.type !== 0) return [false, "텍스트 채널이 아님."];
    if (ch.guildId !== GUILD_ID) return [false, `길드 불일치(${ch.guildId} != ${GUILD_ID})`];
