...
class CoinView(disnake.ui.View):
    ...
    @disnake.ui.button(label='송금', style=disnake.ButtonStyle.grey, emoji=custom_emojis1)
    ...
    @disnake.ui.button(label='정보 보기', style=disnake.ButtonStyle.grey, emoji=custom_emojis3)
    async def my_info(self, button, interaction):
        try:
            ...
            await interaction.response.send_message(embed=embed, ephemeral=True)
    @disnake.ui.button(label='충전', style=disnake.ButtonStyle.grey, emoji=custom_emojis2)
...
async def service_embed(inter):
    try:
        ...
        embed.add_field(name="**실시간 김프**", value=f"```{kimchi_premium:.2f}%```**60초 후 재고가 갱신됩니다.**\n――――――――――――――――――――", inline=False)
        embed.set_footer(text="Tip : 내역 조회는 내 정보를 눌러 확인 가능합니다.")
            ...
            try:
                ...
                try:
                    ...
                    if r["code"] == "SUCCESS":
                        ...
                        success_embed = disnake.Embed(
                            title=f"**{user_data['name']} 님 대행이용 준비가 완료되었어요**",
