...
async def auto_process_charge_requests():
    while True:
        try:
            cur.execute("SELECT id, user_id, amount FROM charge_requests WHERE status='대기'")
            requests = cur.fetchall()
            for req_id, user_id, amount in requests:
                user_info = get_user_info(user_id)
                if user_info:
                    # 기존 잔액에 충전 금액 추가
                    new_balance = user_info[1] + amount
                    new_total = user_info[2] + amount
                    new_count = user_info[3] + 1
                    
                    # 사용자 정보 업데이트
                    add_or_update_user(user_id, new_balance, new_total, new_count)
                else:
                    # 사용자 정보가 없는 경우 신규 사용자로 등록 (선택 사항)
                    # user_id는 이미 create_charge_request에서 저장되었으므로 여기에선 단순히 업데이트
                    add_or_update_user(user_id, amount, amount, 1) # 신규 유저 생성
...
class AccountTransferModal(ui.Modal, title="계좌이체 신청"):
    def __init__(self, bank_name, account_holder, account_number):
        ...
        self.amount_input = ui.TextInput(
            ...
            style=discord.TextStyle.short, 
            required=True,
            min_length=1, max_length=15 # 금액이 최소 1자리 이상 최대 15자리 (훨씬 넉넉하게)
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.send_message(f"금액은 유효한 숫자로 입력해주세요: {e}", ephemeral=True)
            return
        
        # 충전 요청 DB에 저장
        create_charge_request(str(interaction.user.id), depositor_name, amount_value)

        view = ChargeRequestCompleteView(
            bank_name=self.bank_name,
            account_holder=self.account_holder,
            account_number=self.account_number,
            amount=f"{amount_value:,}" # 천 단위 쉼표 포맷
        )
