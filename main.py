...
# -----------------------------------
# 2️⃣ 정보 조회 함수 (성공했던 3단 우회 로직 그대로)
# -----------------------------------
def fetch_gamepass_details(pass_id):
    ...
    try:
        ...
        if res.status_code == 200:
            data = res.json()
            raw_name = data.get("Name") or data.get("name") or "Unknown"
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if match:
                ...
                raw_name = info.get("name") or info.get("Name") or "Unknown"
...
# -----------------------------------
# 3️⃣ 구매 실행 함수 (InvalidArguments 해결 버전)
# -----------------------------------
def purchase_gamepass(product_id, expected_price, seller_id):
    ...
    if not row or not row[0]: return {"success": False, "message": "쿠키가 설정되지 않았습니다."}
    ...
    try:
        ...
        if not csrf_token: return {"success": False, "message": "CSRF 토큰 획득 실패"}
        ...
        if res.status_code == 200:
            ...
            return {"success": False, "message": data.get("reason") or "잔액 부족 또는 중복 소유"}
        return {"success": False, "message": f"서버 오류: {res.status_code}"}
    except Exception as e:
        return {"success": False, "message": str(e)}

# -----------------------------------
# 4️⃣ 구매 확인 및 버튼 콜백 (컨테이너 UI)
# -----------------------------------
