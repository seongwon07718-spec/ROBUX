...
class InMemoryStore:
    def __init__(self):
        ...
        self.wallets: Dict[int, Dict[str, Any]] = {}   # user_id -> {balance,total,rank}
        self.orders: Dict[str, Dict[str, Any]] = {}    # order_id -> ...
        self.topups: Dict[str, Dict[str, Any]] = {}    # req_id -> ...
...
class RobuxBot(commands.Bot):
    async def setup_hook(self):
        ...
        logger.info("슬래시 동기화 완료 (DB 미사용 버전)")
...
async def update_panel_message(guild: discord.Guild):
    ...
    try:
        ...
        await message.edit(embed=await build_panel_embed(), view=PanelView())
    except Exception as e:
...
class RobuxCog(commands.Cog):
    ...
    async def on_interaction(self, inter: Interaction):
        ...
        if cid == "btn_charge":
            ...
            if admin_ch_id and inter.guild:
                try:
                    admin_ch = inter.guild.get_channel(admin_ch_id) or await inter.guild.fetch_channel(admin_ch_id)
                    t_emb = Embed(
                        description=(
                            f"{str(E['tiny_id'])}신청자 : `{inter.user.display_name}`\n"
                            f"{str(E['info1'])}충전금액 : `미입력(예시 10000)`"
                        ),
                        color=COLOR_GREY
                    )
                    view = discord.ui.View(timeout=None)

                    async def approve_topup(i2: Interaction):
                        for c in view.children:
                            if isinstance(c, discord.ui.Button): c.disabled = True
                        await admin_msg.edit(embed=Embed(description="승인됨", color=COLOR_GREEN), view=view)
                        await i2.followup.send("승인 처리 완료", ephemeral=True)

                    async def deny_topup(i2: Interaction):
                        for c in view.children:
                            if isinstance(c, discord.ui.Button): c.disabled = True
                        await admin_msg.edit(embed=Embed(description="거부됨", color=COLOR_RED), view=view)
                        await i2.followup.send("거부 처리 완료", ephemeral=True)
                    ...
                    b1.callback = approve_topup; b2.callback = deny_topup
                    view.add_item(b1); view.add_item(b2)

                    admin_msg = await admin_ch.send(embed=t_emb, view=view)
                except Exception as e:
                    logger.warning(f"충전 관리자 카드 전송 실패: {e}")
            return
    ...
    async def on_message(self, message: discord.Message):
        ...
        if not m:
            try:
                await message.channel.send("정확한 수량을 숫자만 입력해서 보내주세요.")
            except:
                pass
            return
        ...
        qty = int(m.group(0))
        try:
            await message.channel.send("조금만 기다려주세요.")
        except:
            pass
        ...
        if not admin_ch_id:
            try:
                await message.channel.send("관리자 채널이 아직 설정되지 않았어요.")
            except:
                pass
            return
        ...
        if not admin_ch:
            try:
                await message.channel.send("관리자 채널을 찾을 수 없어요.")
            except:
                pass
            return
        ...
        async def approve_cb(inter: Interaction):
            try:
                ...
                await inter.followup.send("재고가 부족합니다. 먼저 재고를 보충해주세요.", ephemeral=True)
                return
...
async def cmd_send_panel(inter: Interaction):
    if not await is_admin_interaction(inter):
        await inter.response.send_message("관리자만 사용 가능합니다.", ephemeral=True); return
    await inter.response.defer(ephemeral=True, thinking=False)
    emb = await build_panel_embed()
    msg = await inter.channel.send(embed=emb, view=PanelView())
    db.config["panel_channel_id"] = inter.channel.id
    db.config["panel_message_id"] = msg.id
    await inter.followup.send("패널이 전송되었습니다.", ephemeral=True)
...
async def cmd_set_price(inter: Interaction, 가격: int):
    ...
    await inter.response.defer(ephemeral=True, thinking=False)
    if 가격 < 0:
        await inter.followup.send("0 이상 정수만 가능합니다.", ephemeral=True); return
    db.config["price_per_unit"] = 가격
    await inter.followup.send(embed=Embed(description=f"{str(E['thumbsuppp'])}1당 `{가격}`으로 설정되었습니다", color=COLOR_GREY), ephemeral=True)
...
async def cmd_set_stock(inter: Interaction, 수량: int, 방식: app_commands.Choice[str]):
    ...
    await inter.response.defer(ephemeral=True, thinking=False)
    if 수량 < 1:
        await inter.followup.send("수량은 1 이상 정수만 가능합니다.", ephemeral=True); return
    try:
        if 방식.value == "추가":
            db.add_stock(수량)
            emb = Embed(description=f"{str(E['thumbsuppp'])}`{수량}`로벅스가 추가되었습니다", color=COLOR_GREEN)
        else:
            db.sub_stock(수량)
            emb = Embed(description=f"{str(E['thumbsuppp'])}`{수량}`로벅스가 차감되었습니다", color=COLOR_RED)
    except ValueError:
...
async def cmd_user_info(inter: Interaction, 유저: discord.User):
    ...
    desc = f"{str(E['big_id'])}남은 잔액 : `{w['balance']}`\n\n{str(E['upuoipipi'])}누적 금액 : `{w['total']}`\n\n{str(E['role_glass'])}역할 등급 : `{w['rank']}`"
...
async def cmd_payment_methods(inter: Interaction, 계좌: app_commands.Choice[str], 코인: app_commands.Choice[str], 문상: app_commands.Choice[str]):
    ...
    desc = (
        f"{str(E['TOSS'])}계좌이체 : `{계좌.value}`\n\n"
        f"{str(E['emoji_68'])}코인결제 : `{코인.value}`\n\n"
        f"{str(E['culture'])}문화상품권 : `{문상.value}`\n-# 설정 완료되었습니다"
    )
...
async def cmd_add_balance(inter: Interaction, 유저: discord.User, 금액: int):
    ...
    await inter.response.defer(ephemeral=True, thinking=False)
    if 금액 < 1:
        await inter.followup.send("금액은 1 이상 정수만 가능합니다.", ephemeral=True); return
    old, added, new = db.add_balance(유저.id, 금액)
    desc = f"금액 : `{금액}`\n\n{str(E['list'])}기존 금액 : `{old}`\n\n{str(E['list'])}충전 금액 : `{added}`\n\n{str(E['list'])}충전 후 금액 : `{new}`"
...
async def cmd_sub_balance(inter: Interaction, 유저: discord.User, 금액: int):
    ...
    desc = f"금액 : `{금액}`\n\n{str(E['list'])}기존 금액 : `{old}`\n\n{str(E['list'])}차감 금액 : `{sub}`\n\n{str(E['list'])}차감 후 금액 : `{new}`"
...
async def cmd_edit_account(inter: Interaction, 은행명: str, 계좌번호: str, 예금주: str, 유효시간분: Optional[int] = None):
    ...
    desc = (
        f"{str(E['__'])}은행명 : `{은행명}`(으)로 변경되었습니다\n\n"
        f"{str(E['__'])}계좌번호 : `{계좌번호}`(으)로 변경되었습니다\n\n"
        f"{str(E['__'])}예금주 : `{예금주}`(으)로 변경되었습니다\n\n"
        f"{str(E['__'])}유효시간 : `{fmt_mmss(expire)}`(으)로 변경되었습니다"
    )
    await inter.followup.send(embed=Embed(description=desc, color=COLOR_GREY), ephemeral=True)
...
async def cmd_log_setting(inter: Interaction, 관리자채널: discord.TextChannel, 후기로그: app_commands.Choice[str], 구매로그: app_commands.Choice[str]):
    ...
    desc = (
        f"{str(E['warn_bell'])}이용로그 : `{bool_label(db.logs['purchase_log_enabled'])}`\n\n"
...
async def cmd_revenue_view(inter: Interaction):
    ...
    await inter.response.defer(ephemeral=True, thinking=False)
    desc = (
        f"{str(E['upuoipipi'])}이번주 총 `0`만큼 벌었습니다\n\n"
        f"{str(E['upuoipipi'])}한달 총 `0`만큼 벌었습니다\n\n"
        f"{str(E['upuoipipi'])}종합 총 `0`만큼 벌었습니다\n"
        f"ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ\n"
        f"{str(E['upuoipipi'])}이번주 총 `0`만큼 팔았습니다\n\n"
        f"{str(E['upuoipipi'])}한달 총 `0`만큼 팔았습니다\n\n"
        f"{str(E['upuoipipi'])}종합 총 `0`만큼 팔았습니다\n"
        f"-# 수익 & 판매량 표시"
    )
    await inter.followup.send(embed=Embed(description=desc, color=COLOR_GREY), ephemeral=True)
