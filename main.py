...
async def auto_process_charge_requests():
    ...
    await asyncio.sleep(30)  # 30초마다 확인=== UI 뷰 클래스 정의 ===
충전 완료 컨테이너 뷰
...
async def munsang_button_callback(self, interaction: discord.Interaction):
    await interaction.response.send_message(view=ErrorMessageView("문상결제 기능은 아직 구현되지 않았습니다."), ephemeral=True)유저 정보 뷰
...
class MyLayout(ui.LayoutView):
    def init(self):
        super().init(timeout=None) # 뷰 만료 시간 없음
...
async def button_4_callback(self, interaction: discord.Interaction):
    ...
    if not await check_vending_access(user_id):
        await interaction.response.send_message(view=VendingBanView(), ephemeral=True)
        return
    await interaction.response.send_message(view=ErrorMessageView("구매 기능은 아직 구현되지 않았습니다."), ephemeral=True)=== 모달 클래스 정의 ===
계좌 설정 모달
...
async def on_submit(self, interaction: discord.Interaction):
    ...
    await interaction.response.send_message(view=BankAccountSetView(bank_name, account_holder, account_number), ephemeral=True)계좌 이체 모달 (충전 신청용)
...
async def on_submit(self, interaction: discord.Interaction):
    ...
    await interaction.response.send_message(view=ChargeRequestCompleteView(self.bank_name, self.account_holder, self.account_number, f"{amount_value:,}"), ephemeral=True)=== 일반적인 오류 메시지 뷰 ===
class ErrorMessageView(ui.LayoutView):
    def init(self, message="오류가 발생했습니다. 다시 시도해주세요."):
        super().init(timeout=None)
        c = ui.Container()
...
if ban_status.value == 'o':
    ...
    await interaction.response.send_message(view=UnbanSetView(target_user.name), ephemeral=True)@bot.tree.command(name="결제수단_설정", description="봇에서 지원할 결제 수단을 설정합니다.")
@discord.app_commands.describe(
...
return jsonify({"status": "success", "message": "충전 요청이 성공적으로 등록되었습니다. 잠시 후 처리됩니다."}), 200Flask 앱을 별도의 스레드에서 실행하는 함수
def run_flask():
...
# 디스코드 봇 시작
bot.run("YOUR_BOT_TOKEN")  # 여기에 봇 토큰을 입력하세요if name == "main":
    main()
