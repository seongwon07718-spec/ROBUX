...
def _check(i: discord.Interaction):
    if not is_registered(i.guild.id): return "not_reg"
    if not is_guild_admin(i.user):    return "not_admin"
    return None


def _no_reg():
    class V(discord.ui.LayoutView):
        c = discord.ui.Container(discord.ui.TextDisplay("등록되지 않은 서버입니다. `/등록`을 먼저 사용하세요."), accent_color=0xED4245)
...
class RoleModal(discord.ui.Modal, title="역할 설정"):
    ...
    async def on_submit(self, i: discord.Interaction):
        ...
        for rtype, field in [("구매자",self.buyer),("VIP",self.vip),("VVIP",self.vvip),("SVIP",self.svip),("리셀러",self.resell)]:
            if field.value.strip(): gdb.set_role(rtype, field.value.strip())
        class V(discord.ui.LayoutView):
            c = discord.ui.Container(discord.ui.TextDisplay("## 역할 설정 완료\n저장되었습니다."), accent_color=0x57F287)
...
class BasicModal(discord.ui.Modal, title="기본 설정"):
    ...
    async def on_submit(self, i: discord.Interaction):
        ...
        class V(discord.ui.LayoutView):
            c = discord.ui.Container(discord.ui.TextDisplay("## 기본 설정 완료"), accent_color=0x57F287)
        await i.response.send_message(view=V(timeout=None), ephemeral=True)
    ...
    async def on_submit(self, i: discord.Interaction):
        try:
            ...
            class Ve(discord.ui.LayoutView):
                c = discord.ui.Container(discord.ui.TextDisplay("금액은 숫자여야 합니다."), accent_color=0xED4245)
...
class AntiThirdModal(discord.ui.Modal, title="3자 방지 코드 설정"):
    ...
    async def on_submit(self, i: discord.Interaction):
        ...
        if not v.isalnum():
            class Ve(discord.ui.LayoutView):
                c = discord.ui.Container(discord.ui.TextDisplay("영숫자 4자리만 입력 가능합니다."), accent_color=0xED4245)
            await i.response.send_message(view=Ve(timeout=None), ephemeral=True); return
        GuildDB(self.gid).set_cfg("anti_third_code", v)
        class V(discord.ui.LayoutView):
            c = discord.ui.Container(discord.ui.TextDisplay(f"## 3자 방지 설정 완료\n코드 : `{v}`"), accent_color=0x57F287)
        await i.response.send_message(view=V(timeout=None), ephemeral=True)
...
class LogModal(discord.ui.Modal, title="로그 채널 설정 (채널 ID)"):
    ...
    def __init__(self, gid, bot): super().__init__(); self.gid = gid; self.bot = bot
    async def on_submit(self, i: discord.Interaction):
        await i.response.defer(ephemeral=True)
        gdb = GuildDB(self.gid)
        results = []
        pairs = [("charge_log",self.charge_ch),("purchase_log",self.purchase_ch),
                 ("review_log",self.review_ch),("stock_log",self.stock_ch),("admin_log",self.admin_ch)]
        for log_type, field in pairs:
            if not field.value.strip(): continue
            try:
                ch = self.bot.get_channel(int(field.value.strip()))
                if ch and isinstance(ch, discord.TextChannel):
                    url = await make_webhook(ch, f"자판기_{log_type}")
                    if url: gdb.set_wh(log_type, url); results.append(f"#{ch.name} 설정 완료")
                    else:   results.append(f"#{ch.name} 웹훅 생성 실패")
                else: results.append(f"채널 {field.value.strip()} 찾을 수 없음")
            except Exception as e: results.append(f"오류: {e}")
        txt = "\n".join(results) if results else "변경 없음"
        class V(discord.ui.LayoutView):
            c = discord.ui.Container(discord.ui.TextDisplay(f"## 로그 설정 완료\n{txt}"), accent_color=0x57F287)
...
class CatAddModal(discord.ui.Modal, title="카테고리 추가"):
    ...
    async def on_submit(self, i: discord.Interaction):
        ...
        class V(discord.ui.LayoutView):
            c = discord.ui.Container(discord.ui.TextDisplay(txt), accent_color=0x57F287 if ok else 0xED4245)
        await i.response.send_message(view=V(timeout=None), ephemeral=True)
...
class ProdAddModal(discord.ui.Modal, title="상품 추가"):
    ...
    async def on_submit(self, i: discord.Interaction):
        gdb = GuildDB(self.gid)
        try: cid = int(self.cat_id.value); price = int(self.price.value.replace(",",""))
        except ValueError:
            class Ve(discord.ui.LayoutView):
                c = discord.ui.Container(discord.ui.TextDisplay("ID와 가격은 숫자여야 합니다."), accent_color=0xED4245)
            await i.response.send_message(view=Ve(timeout=None), ephemeral=True); return
        cats = {c["id"]: c["name"] for c in gdb.get_cats()}
        if cid not in cats:
            class Ve(discord.ui.LayoutView):
                c = discord.ui.Container(discord.ui.TextDisplay("카테고리를 찾을 수 없습니다."), accent_color=0xED4245)
            await i.response.send_message(view=Ve(timeout=None), ephemeral=True); return
        pid = gdb.add_prod(cid, self.name.value.strip(), price)
        class V(discord.ui.LayoutView):
            c = discord.ui.Container(
                discord.ui.TextDisplay(f"## 상품 추가 완료\n[{pid}] {self.name.value.strip()} | {fmt(price)}\n카테고리: {cats[cid]}"),
                accent_color=0x57F287,
            )
        await i.response.send_message(view=V(timeout=None), ephemeral=True)
...
class StockModal(discord.ui.Modal, title="재고 추가"):
    ...
    async def on_submit(self, i: discord.Interaction):
        gdb = GuildDB(self.gid)
        try: pid = int(self.pid.value.strip())
        except ValueError:
            class Ve(discord.ui.LayoutView):
                c = discord.ui.Container(discord.ui.TextDisplay("상품 ID는 숫자여야 합니다."), accent_color=0xED4245)
            await i.response.send_message(view=Ve(timeout=None), ephemeral=True); return
        p = gdb.get_prod(pid)
        if not p:
            class Ve(discord.ui.LayoutView):
                c = discord.ui.Container(discord.ui.TextDisplay("상품을 찾을 수 없습니다."), accent_color=0xED4245)
            await i.response.send_message(view=Ve(timeout=None), ephemeral=True); return
        ...
        class V(discord.ui.LayoutView):
            c = discord.ui.Container(
                discord.ui.TextDisplay(f"## 재고 추가 완료\n상품: **{p['name']}**\n추가: **{added}개** | 총: **{total}개**"),
                accent_color=0x57F287,
            )
        await i.response.send_message(view=V(timeout=None), ephemeral=True)
...
class ChargeModeView(discord.ui.LayoutView):
    ...
    async def auto_btn(self, i: discord.Interaction, btn: discord.ui.Button):
        ...
        for item in self.walk_children():
            if isinstance(item, discord.ui.Button) and item.label == "수동 충전":
                item.disabled = False
        for item in self.walk_children():
            if isinstance(item, discord.ui.TextDisplay):
                item.content = "## 자동 충전 활성화\n카카오뱅크 자동충전이 활성화됩니다."
    ...
    async def manual_btn(self, i: discord.Interaction, btn: discord.ui.Button):
        ...
        btn.disabled = True
        for item in self.walk_children():
            if isinstance(item, discord.ui.Button) and item.label == "자동 충전":
                item.disabled = False
        for item in self.walk_children():
            if isinstance(item, discord.ui.TextDisplay):
                item.content = "## 수동 충전 활성화\n관리자가 직접 충전을 확인합니다."
        await i.response.edit_message(view=self)
...
class CatDeleteView(discord.ui.LayoutView):
    def __init__(self, gid: int, cats: list):
        ...
        sel = discord.ui.Select(placeholder="삭제할 카테고리 선택", options=options, max_values=min(len(options),5))
        sel.callback = self._do_delete
        self._row = discord.ui.ActionRow(sel)
    ...
    async def _do_delete(self, i: discord.Interaction):
        ...
        for cid_s in i.data["values"]:
            deleted.append(cats.get(cid_s, cid_s))
...
class ProdDelModal(discord.ui.Modal, title="상품 삭제"):
    ...
    async def on_submit(self, i: discord.Interaction):
        ...
        try: pid = int(self.pid.value.strip())
            ...
            class Ve(discord.ui.LayoutView):
                c = discord.ui.Container(discord.ui.TextDisplay("숫자를 입력하세요."), accent_color=0xED4245)
            await i.response.send_message(view=Ve(timeout=None), ephemeral=True); return
        p = gdb.get_prod(pid)
        if not p:
            class Ve(discord.ui.LayoutView):
        ...
        gdb.del_prod(pid)
        class V(discord.ui.LayoutView):
            c = discord.ui.Container(discord.ui.TextDisplay(f"## 상품 삭제 완료\n`{p['name']}` 삭제됨"), accent_color=0x57F287)
        await i.response.send_message(view=V(timeout=None), ephemeral=True)
...
class SettingsMainView(discord.ui.LayoutView):
    def __init__(self, gid: int, bot):
        ...
        self._sel = SettingsSelect(gid, bot)
        self._row = discord.ui.ActionRow(self._sel)
        self.add_item(self._row)
...
class SettingsSelect(discord.ui.Select):
    def __init__(self, gid: int, bot):
        self.gid = gid; self.bot = bot
        super().__init__(placeholder="설정 항목 선택", options=[
    ...
    async def callback(self, i: discord.Interaction):
        ...
        if v == "role":        await i.response.send_modal(RoleModal(gid))
            ...
            class _V(discord.ui.LayoutView):
                c = discord.ui.Container(discord.ui.TextDisplay(f"## 카테고리 목록\n{cat_txt}"), accent_color=0x5865F2)
            await i.response.send_message(view=_V(timeout=None), ephemeral=True)
            ...
            class _V(discord.ui.LayoutView):
                c = discord.ui.Container(discord.ui.TextDisplay(f"## 상품 목록{prod_txt}"), accent_color=0x5865F2)

            class _Vd(discord.ui.LayoutView):
                row: discord.ui.ActionRow = discord.ui.ActionRow()
                @row.button(label="상품 삭제", style=discord.ButtonStyle.danger)
                async def del_btn(s, ii: discord.Interaction, btn: discord.ui.Button):
                    await ii.response.send_modal(ProdDelModal(gid))

            await i.response.send_message(view=_V(timeout=None), ephemeral=True)
            await i.followup.send(view=_Vd(timeout=60), ephemeral=True)

        elif v == "stock":
            gdb = GuildDB(gid); prods = gdb.all_prods()
            if not prods:
                class Ve(discord.ui.LayoutView):
            ...
            class _V(discord.ui.LayoutView):
                c = discord.ui.Container(discord.ui.TextDisplay("## 상품 목록\n" + "\n".join(lines)), accent_color=0x5865F2)
            await i.response.send_message(view=_V(timeout=None), ephemeral=True)
...
class SettingsCog(commands.Cog):
    ...
    async def settings(self, i: discord.Interaction):
        err = _check(i)
        if err == "not_reg": await i.response.send_message(view=_no_reg(),   ephemeral=True); return
        if err == "not_admin": await i.response.send_message(view=_no_admin(), ephemeral=True); return
