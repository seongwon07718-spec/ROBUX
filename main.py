...
EMOJI_BUY    = "<:Nitro:1422614999804809226>"  # 요청: 구매 버튼 이모지 교체
...
# ===== 잔액 저장소 =====
class BalanceStore:
    # balances[guild_id][user_id] = int(원)
    ...
    def get(cls, gid:int, uid:int) -> int:
        cls._ensure(gid)
        return cls.balances[gid].get(uid, 0)
    @classmethod
    def add(cls, gid:int, uid:int, amount:int) -> tuple[int,int,int]:
        cls._ensure(gid)
        prev = cls.get(gid, uid)
        after = prev + max(0, amount)
        cls.balances[gid][uid] = after
        return prev, amount, after
    @classmethod
...
def build_usage_purchase_embed(user: discord.User, product: str, qty: int)->discord.Embed:
    ...
    emb.set_footer(text="구매 시간"); emb.timestamp = discord.utils.utcnow()
    return emb
...
def build_review_embed(product: str, stars: int, content: str)->discord.Embed:
    ...
    emb.set_footer(text="작성 시간"); emb.timestamp = discord.utils.utcnow()
    return emb
...
class QuantityModal(discord.ui.Modal, title="수량 입력"):
    ...
    def __init__(self, owner_id: int, category: str, product_name: str):
        super().__init__()
        self.owner_id=owner_id; self.category=category; self.product_name=product_name
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not qty_s.isdigit() or int(qty_s)<=0:
            await interaction.response.send_message("수량은 1 이상의 숫자여야 합니다.", ephemeral=True); return
        qty=int(qty_s)
        prod = ProductStore.get(self.product_name, self.category)
        if not prod:
            await interaction.response.send_message("유효하지 않은 제품입니다.", ephemeral=True); return
        if prod["stock"] < qty:
            await interaction.response.send_message("재고가 부족합니다.", ephemeral=True); return
        ...
        await interaction.response.send_message(embed=discord.Embed(title="구매 완료", description=f"{self.product_name} {qty}개 구매가 처리됐습니다. DM을 확인해주세요.", color=GRAY), ephemeral=True)
...
class ReviewModal(discord.ui.Modal, title="구매 후기 작성"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not stars_s.isdigit():
            await interaction.response.send_message("별점은 숫자(1~10)만 입력해줘.", ephemeral=True); return
        stars=int(stars_s)
        if stars<1 or stars>10:
            await interaction.response.send_message("별점은 1~10 사이여야 해.", ephemeral=True); return
        ...
        await interaction.response.send_message("후기 고마워! 채널에 공유됐어.", ephemeral=True)
...
class CategorySetupModal(discord.ui.Modal, title="카테고리 추가"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(embed=discord.Embed(title="카테고리 등록 완료", description=f"{(prev+' ') if prev else ''}{name}\n{desc}", color=GRAY), ephemeral=True)
...
class CategoryRootView(discord.ui.View):
    def __init__(self, owner_id:int):
        super().__init__(timeout=None); self.add_item(CategoryRootSelect(owner_id))
...
class ProductRootView(discord.ui.View):
    def __init__(self, owner_id:int):
        super().__init__(timeout=None); self.add_item(ProductRootSelect(owner_id))
...
class ControlCog(commands.Cog):
    ...
    async def 잔액_설정(self, interaction: discord.Interaction,
                    유저: discord.Member,
                    금액: int,
                    여부: app_commands.Choice[str]):

        if 금액 < 0:
            await interaction.response.send_message("금액은 음수가 될 수 없어.", ephemeral=True); return

        gid = interaction.guild.id
        uid = 유저.id

        if 여부.value == "차감":
            prev, sub_amt, after = BalanceStore.sub(gid, uid, 금액)
            emb = discord.Embed(
                title=f"{유저} 금액 차감",
                description=f"원래 금액 : {prev}\n차감 할 금액 : {sub_amt}\n차감 후 금액 : {after}",
                color=RED
            )
            emb.set_footer(text="변경 시간"); emb.timestamp = discord.utils.utcnow()
            await interaction.response.send_message(embed=emb, ephemeral=True)
        else:
            prev, add_amt, after = BalanceStore.add(gid, uid, 금액)
            emb = discord.Embed(
                title=f"{유저} 금액 추가",
                description=f"원래 금액 : {prev}\n추가 할 금액 : {add_amt}\n추가 후 금액 : {after}",
                color=GREEN
