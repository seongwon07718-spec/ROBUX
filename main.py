...
from threading import Thread  # 추가
...
# 입금 내역을 임시 저장할 딕셔너리 (메모리 보관)
pending_deposits = {}

@app.post("/charge")
async def receive_charge(data: ChargeData):
    """아이폰 단축어에서 보낸 메시지를 수신"""
    msg = data.message.strip()
    print(f"📥 [입금 문자 수신]: {msg}")
    
    # 메시지에서 이름과 금액 추출 시도 (예: 홍길동 10000)
    match = re.search(r'([가-힣]+)\s*(\d+)', msg)
    if match:
        name = match.group(1)
...
# [do_culture_charge, CultureModal, AdminLogView, BankInfoLayout 클래스 생략 - 기존과 동일]
...
class BankModal(ui.Modal, title="계좌이체 충전"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # ✅ 수정된 로직: 통합된 내부 pending_deposits에서 내역 확인
        ...
        if pending_deposits.get(key):
            ...
            del pending_deposits[key] # 확인 완료 후 삭제
...
# [ChargeLayout, MeuLayout, vending 명령어 생략 - 기존과 동일]
...
class MeuLayout(ui.LayoutView):
    def __init__(self):
        ...
        container.add_item(ui.Separator(spacing=discord.SeparatorSpacing.small))
        shop = ui.Button(label="제품"); shop.callback = self.shop_callback
        chage = ui.Button(label="충전"); chage.callback = self.chage_callback
        buy = ui.Button(label="구매"); buy.callback = self.buy_callback
        info = ui.Button(label="정보"); info.callback = self.info_callback
        container.add_item(ui.ActionRow(shop, chage, buy, info))
        container.add_item(ui.Separator(spacing=discord.SeparatorSpacing.small)); container.add_item(ui.TextDisplay("-# © 2026 Made by **@alx.io07** All rights reserved"))
    ...
    async def info_callback(self, it):
        ...
        container.add_item(ui.Separator(spacing=discord.SeparatorSpacing.small)); container.add_item(ui.TextDisplay(f"보유 잔액: ₩{money:,}\n누적 금액: ₩{total_spent:,}"))
        selecao = ui.Select(placeholder="거래 내역 조회하기", options=[discord.SelectOption(label="최근 구매 내역", value="buy", emoji="🛒"), discord.SelectOption(label="최근 충전 내역", value="charge", emoji="💳")])
        async def resp(i):
...
if __name__ == "__main__":
    ...
    # 봇 실행
    bot.run("YOUR_BOT_TOKEN_HERE") # 본인의 봇 토큰 입력
