    ...
    def get_user_places(self, user_id: int) -> list[dict]:
        ...
        while True:
            ...
            for g in body.get("data", []):
                ...
                root_place_id = root_place.get("id") 
...
class PlaceSelectView(ui.LayoutView):
    ...
    async def on_select(self, it: discord.Interaction):
        ...
        await it.response.edit_message(
            view=await get_container_view("<a:1792loading:1487444148716965949>  조회 중", "게임패스 목록을 불러오는 중입니다", 0x5865F2)
        )
        loop = asyncio.get_running_loop()
        api = RobloxAPI()
        ...
        if not passes:
            await it.edit_original_response(
                view=await get_container_view("<:downvote:1489930277450158080>  결과 없음", " - 판매 중인 게임패스가 없습니다", 0xED4245)
            )
            return
...
class CookieModal(ui.Modal, title="로블록스 쿠키 등록"):
    ...
    async def on_submit(self, it: discord.Interaction):
        ...
        if is_success:
            ...
            con = create_container_msg("<:upvote:1489930275868770305>  쿠키 등록 성공", f"-# - 계정에 접속을 성공했습니다\n-# - 현재 재고: **{result:,}로벅스**", 0x57F287)
            await it.response.send_message(view=ui.LayoutView().add_item(con), ephemeral=True)
        else:
            con = create_container_msg("<:downvote:1489930277450158080>  쿠키 등록 실패", f"-# - 인증에 실패하였습니다\n-# - 사유: `{result}`", 0xED4245)
            await it.response.send_message(view=ui.LayoutView().add_item(con), ephemeral=True)
...
class RobuxVending(ui.LayoutView):
    ...
    async def build_main_menu(self):
        ...
        con.add_item(ui.TextDisplay("-# - **게임패스 방식** / 무조건 본인 게임만 가능\n-# - **글로벌 선물 방식** / 예시: 라이벌 - 번들\n-# - **그룹 지급 방식** / 그룹 2주 동안 참가시 가능"))
        con.add_item(ui.Separator(spacing=discord.SeparatorSpacing.small))
        ...
        row_btns = ui.ActionRow(charge, info, shop, calc)
    ...
    async def main_callback(self, it: discord.Interaction):
        ...
        if cid == "charge":
            con = ui.Container()
            con.accent_color = 0x5865F2
            con.add_item(ui.TextDisplay("### <:emoji_19:1487441741484392498>  충전 수단\n-# - 원하시는 **충전 방식**을 선택해주세요\n-# - 현재 가능한 결제 수단은 **계좌이체**"))
            con.add_item(ui.Separator(spacing=discord.SeparatorSpacing.small))
...
async def set_rate(it: discord.Interaction, 수량: int):
    ...
    await it.response.send_message(f"-# - **가격 설정 완료**: 1.0당 {수량:,}로벅스", ephemeral=True)
...
if __name__ == "__main__":
    with sqlite3.connect("robux_shop.db") as conn:
        ...
        """)
        conn.commit()
    print("Auto Robux Vending Bot is running...")
