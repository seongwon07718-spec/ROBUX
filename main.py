...
def star_bar_or_none(avg: float | None) -> str:
    ...
    n = max(1, min(int(round(avg)), 5))   # 최소 1개는 보이게
    return "⭐️"*n
...
class ReviewModal(discord.ui.Modal, title="구매 후기 작성"):
    ...
    async def on_submit(self, it: discord.Interaction):
        ...
        if not s.isdigit(): await it.response.send_message("별점은 1~5 사이 숫자만 입력해줘.", ephemeral=True); return
        stars=int(s)
        if stars<1 or stars>5: await it.response.send_message("별점은 1~5 사이여야 해.", ephemeral=True); return
...
class CategorySelectForBuy(discord.ui.Select):
    ...
    async def callback(self, it:discord.Interaction):
        ...
        view  = BuyFlowView(self.owner_id, val)
        try:
...
class LogChannelIdModal(discord.ui.Modal, title="로그 채널 설정"):
    ...
    async def on_submit(self, it):
        ...
        title_map={"purchase":"구매로그","review":"구매후기","admin":"관리자로그"}
        await it.response.send_message(embed=discord.Embed(title=f"{title_map[self.log_key]} 채널 지정 완료", description=f"목적지: {ch.mention}", color=GRAY), ephemeral=True)
