...
# -----------------------------------
# 3️⃣ [해결 포인트] 실구매 함수 (410 에러 방지용 전용 경로)
# -----------------------------------
async def purchase_gamepass(pass_id, expected_price, seller_id):
    """
    410 Gone 에러를 해결하기 위해 해외에서 사용하는 
    game-passes 전용 구매 엔드포인트와 정밀 헤더를 적용했습니다.
    """
    def _purchase():
        conn = sqlite3.connect(DATABASE)
        cur = conn.cursor()
        ...
        if not row or not row[0]: return {"success": False, "message": "관리자 쿠키가 없습니다."}

        session = requests.Session()
        session.cookies.set(".ROBLOSECURITY", row[0], domain=".roblox.com")
        
        # [중요] 실제 브라우저와 동일한 수준의 헤더 구성
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36",
        ...
        try:
            ...
            if not csrf: return {"success": False, "message": "CSRF 토큰 갱신 실패"}
            headers["X-CSRF-TOKEN"] = csrf

            # [410 해결 핵심] 
            # 일반 상품 경로(/v1/purchases/products/) 대신 
            # 게임패스 전용 경로(/v1/purchases/game-passes/)를 사용해야 410을 피할 수 있는 경우가 많습니다.
            ...
            if res.status_code == 200:
                ...
                if reason == "AlreadyOwned": reason = "이미 소유한 아이템"
                if reason == "InsufficientFunds": reason = "관리자 계정 잔액 부족"
                return {"success": False, "message": reason}
            
            # 만약 위 경로도 410이 뜬다면, 마지막 수단으로 product_id 경로 재시도 (조회 시 가져온 것)
...
class GamepassConfirmView(ui.LayoutView):
    ...
    async def self_confirm(self, it: discord.Interaction):
        ...
        await it.response.edit_message(view=get_container_view("⌛ 처리 중", "로블록스 보안 우회 결제 진행 중...", 0xFEE75C))

        # 게임패스 전용 구매 함수 호출 (pass_id를 직접 사용)
        result = await purchase_gamepass(self.info['id'], self.info['price'], self.info.get('sellerId'))
...
class GamepassModal(ui.Modal, title="게임패스 구매"):
    ...
    async def on_submit(self, it: discord.Interaction):
        ...
        if not info:
            return await it.followup.send(view=get_container_view("❌ 조회 실패", "상품 정보를 불러올 수 없습니다.", 0xED4245), ephemeral=True)
