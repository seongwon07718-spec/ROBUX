...
class CoinView(discord.ui.View):
    ...
    @discord.ui.button(label='송금', style=discord.ButtonStyle.grey, emoji=custom_emojis1, custom_id="use_service_button_delegated")
    ...
    @discord.ui.button(label='정보 보기', style=discord.ButtonStyle.grey, emoji=custom_emojis3, custom_id="my_info_button_delegated")
    async def my_info(self, button, interaction):
        try:
            ...
            view = discord.ui.View()
            history_btn = discord.ui.Button(label="거래내역", style=discord.ButtonStyle.grey, custom_id="view_history")
    ...
    @discord.ui.button(label='충전', style=discord.ButtonStyle.grey, emoji=custom_emojis2, custom_id="charge_button_delegated")
...
class ServiceContainerView(ui.LayoutView):
    def __init__(self, stock_display: str, kimchi_premium_display: str):
        ...
        c.add_item(ui.Separator(spacing=discord.SeparatorSpacing.small)) # 구분선(막대기)
        self.add_item(c)

        stock_kimchi_container = ui.Container()
        stock_btn = ui.Button(label=f"실시간 재고: {stock_display}", style=discord.ButtonStyle.grey, disabled=True)
        kimchi_btn = ui.Button(label=f"실시간 김프: {kimchi_premium_display}", style=discord.ButtonStyle.grey, disabled=True)
        stock_kimchi_container.add_item(stock_btn)
        stock_kimchi_container.add_item(kimchi_btn)
        self.add_item(stock_kimchi_container)

        self.add_item(ui.Separator(spacing=discord.SeparatorSpacing.small)) # 구분선(막대기)
        ...
        send_button = ui.Button(label="송금", style=discord.ButtonStyle.grey, emoji=custom_emojis1, custom_id="use_service_button")
        info_button = ui.Button(label="정보 보기", style=discord.ButtonStyle.grey, emoji=custom_emojis3, custom_id="my_info_button")
        charge_button = ui.Button(label="충전", style=discord.ButtonStyle.grey, emoji=custom_emojis2, custom_id="charge_button")
        ...
        tail_container.add_item(ui.Separator(spacing=discord.SeparatorSpacing.small)) # 구분선(막대기)
...
@bot.tree_command(name="관리자", description="관리자 추가 / 해제")
async def staff_cmd(inter, 옵션: str = commands.Param(choices=["추가", "해제"]), 유저: discord.Member = None):
    try:
        ...
        if 옵션 == "추가":
            ...
            embed = discord.Embed(color=0x26272f)
...
@bot.tree_command(name="강제인증", description="고객님 강제인증")
async def force_verify(inter, 유저: discord.Member):
    try:
        if inter.author.id not in ALLOWED_USER_IDS:
            embed = discord.Embed(
                title="**접근 거부**",
...
@bot.tree_command(name="블랙", description="인증유저님 블랙하기")
async def blk_user(inter, 유저: discord.Member):
    try:
        if inter.author.id not in ALLOWED_USER_IDS:
            embed = discord.Embed(
                title="**접근 거부**",
...
@bot.tree_command(name="충전거절", description="자동충전 요청 거절")
async def reject_charge(inter, 유저: discord.Member, 금액: int, 사유: str = "사유 없음"):
    try:
        if inter.author.id not in ALLOWED_USER_IDS:
            embed = discord.Embed(
                title="**접근 거부**",
...
@bot.tree_command(name="충전승인", description="자동충전 요청 승인")
async def approve_charge(inter, 유저: discord.Member, 금액: int):
    try:
        if inter.author.id not in ALLOWED_USER_IDS:
            embed = discord.Embed(
                title="**접근 거부**",
...
@bot.tree_command(name="고객충전", description="인증고객님 충전")
async def chrg_user(inter, 유저: discord.Member, 금액: int):
    try:
        if inter.author.id not in ALLOWED_USER_IDS:
            embed = discord.Embed(
                title="**접근 거부**",
...
@bot.tree_command(name="정보조회", description="인증 정보 조회")
async def info_lookup(inter, 유저: discord.Member):
    try:
        if inter.author.id not in ALLOWED_USER_IDS:
            embed = discord.Embed(
                title="**접근 거부**",
...
@bot.tree_command(name="인증해체", description="인증 해제")
async def unverify_user(inter, 유저: discord.Member):
    try:
        if inter.author.id not in ALLOWED_USER_IDS:
            embed = discord.Embed(
                title="**접근 거부**",
...
@bot.tree_command(name="txid조회", description="사용자의 TXID 내역 조회")
async def txid_lookup(inter, 유저: discord.Member):
    try:
        if inter.author.id not in ALLOWED_USER_IDS:
            embed = discord.Embed(
                title="**접근 거부**",
...
@bot.tree_command(name="대행임베드", description="대행 서비스 UI 출력")
async def service_embed(inter: discord.ApplicationCommandInteraction):
    try:
        ...
        if inter.author.id not in ALLOWED_USER_IDS:
            ...
            return
        if not check_admin(inter.author.id):
            embed = discord.Embed(title="**오류**", description="권한이 없습니다.", color=0x26272f)
            await inter.edit_original_response(embed=embed)
            return
        ...
        admin_embed = discord.Embed(color=0x26272f)
...
@bot.event
async def on_button_click(interaction: discord.MessageInteraction):
    try:
        # 이미 응답된 상호작용인지 확인
        ...
        if cid == "start_verify":
            ...
            if verified:
                ...
                done_embed = discord.Embed(title="✅ 이미 본인인증이 완료되었습니다", description="다시 인증하실 필요가 없습니다.", color=0x26272f)
                await interaction.edit_original_response(embed=done_embed)
                return
            if not interaction.response.is_done():
                await interaction.response.defer(ephemeral=True)
            embed = discord.Embed(title="이용중이신 통신사를 선택해주세요.", color=0x26272f)
            
            components = [
                discord.ui.Button(label="SKT", style=discord.ButtonStyle.gray, custom_id="SKT"),
                discord.ui.Button(label="KT", style=discord.ButtonStyle.gray, custom_id="KT"),
                discord.ui.Button(label="LG U+", style=discord.ButtonStyle.gray, custom_id="LG"),
                discord.ui.Button(label="알뜰폰", style=discord.ButtonStyle.gray, custom_id="MVNO")
            ]
            ...
            if cid == "SKT":
                ...
                embed = discord.Embed(title="알뜰폰 통신사를 선택해주세요.", color=0x26272f)
                
                components = [
                    discord.ui.Button(label="SKT", style=discord.ButtonStyle.gray, custom_id="SKM"),
                    discord.ui.Button(label="KT", style=discord.ButtonStyle.gray, custom_id="KTM"),
                    discord.ui.Button(label="LG U+", style=discord.ButtonStyle.gray, custom_id="LGM")
                ]
            ...
            if verified:
                if not interaction.response.is_done():
                    await interaction.response.defer(ephemeral=True)
                done_embed = discord.Embed(title="✅ 이미 본인인증이 완료되었습니다", color=0x26272f)
                await interaction.edit_original_response(embed=done_embed)
                return
            ...
            if interaction.author.id not in user_sessions:
                if not interaction.response.is_done():
                    await interaction.response.defer(ephemeral=True)
                warn = discord.Embed(title="세션이 없습니다", description="처음부터 다시 시도해주세요.", color=0x26272f)
            ...
            try:
                ...
                if not transactions:
                    embed = discord.Embed(title="📋 거래내역", description="거래내역이 없습니다.", color=0x26272f)
                    await interaction.edit_original_response(embed=embed)
                    return
                embed = discord.Embed(title=f"{interaction.author.display_name}님의 전체 거래내역", color=0x26272f)
                for i, tx in enumerate(transactions, 1):
                ...
                view = discord.ui.View()
                txid_btn = discord.ui.Button(label="🔗 TXID 조회", style=discord.ButtonStyle.success, custom_id="view_txid")
                view.add_item(txid_btn)
                await interaction.edit_original_response(embed=embed, view=view)
            except Exception as e:
                logger.error(f"거래내역 조회 오류: {e}")
                embed = discord.Embed(title="오류", description="거래내역 조회 중 오류가 발생했습니다.", color=0x26272f)
                try:
                    await interaction.edit_original_response(embed=embed)
                except Exception:
                    pass
            ...
            try:
                transactions = get_transaction_history(interaction.author.id, 100)
                if not transactions:
                    embed = discord.Embed(title="🔗 TXID 조회", description="거래내역이 없습니다.", color=0x26272f)
                    await interaction.edit_original_response(embed=embed)
                    return
                embed = discord.Embed(title="🔗 TXID 전체 조회", description="거래 TXID와 API TXID를 모두 확인하세요", color=0x26272f)
                txid_text = ""
                for i, tx in enumerate(transactions, 1):
                    if tx.get('txid') or tx.get('api_txid'):
                        time_str = tx.get('timestamp', '')
                ...
                embed = discord.Embed(title="오류", description="TXID 조회 중 오류가 발생했습니다.", color=0x26272f)
                try:
                    await interaction.edit_original_response(embed=embed)
                except Exception:
                    pass
        ...
        embed = discord.Embed(title="**오류**", description="처리 중 오류가 발생했습니다.", color=0x26272f)
        try:
            if interaction.response.is_done():
                await interaction.edit_original_response(embed=embed)
            else:
...
async def start_captcha(interaction: discord.ApplicationCommandInteraction, telecom):
    try:
        embed = discord.Embed(title=f"{interaction.author.name}", description="보안코드를 요청하는중입니다.\n╰ 너무 오래걸릴시 다시 시도해주세요.", color=0x26272f)
        ...
        new_embed = discord.Embed(title=f"{interaction.author.name}", description="휴대폰 본인확인 ㅣ 문자(SMS)\n╰ 고객 정보를 2분내에 입력해 주세요", color=0x26272f)
        ...
        button = discord.ui.Button(label="🔐 본인인증", style=discord.ButtonStyle.red, custom_id="본인인증")
        ...
        try:
            embed = discord.Embed(title="**오류**", description="오류가 발생했습니다. 다시 시도해주세요.", color=0x26272f)
            await interaction.edit_original_response(embed=embed)
        except:
            pass

@bot.event
async def on_modal_submit(interaction: discord.ModalInteraction):
    try:
        if interaction.custom_id == "charge_modal":
            ...
            try:
                ...
                if charge_amount < 500:
                    embed = discord.Embed(title="**오류**", description="최소 충전 금액은 500원입니다.", color=0x26272f)
                    await interaction.edit_original_response(embed=embed)
                    return
                
                embed = discord.Embed(title="**💳 충전 안내**", description=f"충전 요청 금액: **₩{charge_amount:,}**", color=0x26272f)
                ...
                embed = discord.Embed(title="**오류**", description="올바른 금액을 입력해주세요.", color=0x26272f)
                await interaction.edit_original_response(embed=embed)
            return
        ...
        if interaction.custom_id.startswith("info_modal_"):
            ...
            if not user_data:
                await interaction.response.defer(ephemeral=True)
                embed = discord.Embed(title="**오류**", description="세션이 만료되었습니다. 다시 시도해주세요.", color=0x26272f)
                await interaction.edit_original_response(embed=embed)
                return
            ...
            await interaction.response.defer(ephemeral=True)
            embed = discord.Embed(title=f"{interaction.author.name}", description="휴대폰 본인확인 ㅣ 문자(SMS)\n╰ 고객 정보를 확인중입니다.", color=0x26272f)
            embed.set_thumbnail(interaction.author.display_avatar)
            ...
            try:
                ...
                if datarrr["code"] == "RETRY":
                    error_embed = discord.Embed(title=f"{interaction.author.name}", description=f"본인 확인 실패\n╰ {datarrr['message']}", color=0x26272f)
                    ...
                    success_embed = discord.Embed(title="**문자로 온 인증번호 6자리를 입력해주세요**", color=0x26272f)
                    
                    button = discord.ui.Button(label="📱 입력하기", style=discord.ButtonStyle.gray, custom_id="입력하기")
                ...
                try:
                    embed = discord.Embed(title="**오류**", description="오류가 발생했습니다.", color=0x26272f)
                    await interaction.edit_original_response(embed=embed)
                except:
                    pass
            ...
            try:
                ...
                if not user_data:
                    await interaction.response.defer(ephemeral=True)
                    embed = discord.Embed(title="**오류**", description="세션이 만료되었습니다.", color=0x26272f)
                    await interaction.edit_original_response(embed=embed)
                    return
                ...
                await interaction.response.defer(ephemeral=True)
                embed = discord.Embed(title="인증코드 확인중입니다.", color=0x26272f)
                await interaction.edit_original_response(embed=embed)
                ...
                try:
                    ...
                    if r["code"] == "SUCCESS":
                        ...
                        success_embed = discord.Embed(title=f"**{user_data['name']} 님 대행이용 준비가 완료되었어요**", description="지금 바로 이용해보세요", color=0x26272f)
                    ...
                    try:
                        embed = discord.Embed(title="**오류**", description="인증 중 오류가 발생했습니다.", color=0x26272f)
                        await interaction.edit_original_response(embed=embed)
                    except:
                        pass
...
async def manual_refresh(inter: discord.ApplicationCommandInteraction):
    try:
...
async def change_fee(inter: discord.ApplicationCommandInteraction, 퍼센트: float):
    try:
        if inter.author.id not in ALLOWED_USER_IDS or not check_admin(inter.author.id):
            await inter.response.send_message("권한이 없습니다.", ephemeral=True)
            return
        ...
        if message.guild is None and not message.author.bot:
            try:
                ...
                if forward_ch is None:
                    return
                embed = discord.Embed(title="📩 고객 입금 내역 DM 도착", color=0x00c3ff)
                ...
                for att in message.attachments:
                    try:
                        if att.size is None or att.size < 8 * 1024 * 1024:  # 8MB 이하만 재업로드
                            fp = await att.read()
                            files.append(discord.File(BytesIO(fp), filename=att.filename))
                        else:
                ...
                # 사용자에게 접수 알림 DM
                try:
                    ack = discord.Embed(title="💳 충전 신청 완료", description="제출하신 영수증을 확인 중입니다. 검토 후 승인되면 알림드립니다.", color=0x26272f)
                ...
                if waiting_amount:
                    ...
                    req_embed = discord.Embed(title=CHARGE_REQUEST_TITLE, description=desc, color=0x26272f)
                    req_embed.set_footer(text="승인/거절은 1회만 가능합니다.")

                    class ChargeDecisionView(discord.ui.View):
                        ...
                        async def approve(self, button, inter):
                            ...
                            try:
                                ...
                                dm = discord.Embed(title="💳 충전 승인", description=f"₩{self.amount:,} 충전이 승인되었습니다.", color=0x26272f)
                        ...
                        async def deny(self, button, inter):
                            ...
                            class RejectReasonModal(discord.ui.Modal):
                                ...
                                async def callback(modal_inter):
                                    ...
                                    try:
                                        user = await bot.fetch_user(self.user_id)
                                        dm = discord.Embed(title="❌ 충전 거절", description=f"₩{self.amount:,} 충전이 거절되었습니다.", color=0x26272f)
