...
import coin # 'coin' 모듈은 현재 코드에 포함되어 있지 않으므로, 이 파일 외부에 존재한다고 가정합니다.

===== 봇 설정 =====
TOKEN = 'MTQxOTI5NDY4NTQ3MzYL2OT2rf5BGCtBCqXhU8Mguh3C0OU' # 실제 토큰으로 교체해주세요.
DEFAULT_ADMIN_ID = 1202376635128340
# 슬래시 명령어 사용 가능한 사용자 ID (요청: 두 사용자만 허용)
# 주어진 값이 동일하게 중복 제공되어도 한 명만 허용되는 것과 동일하게 동작합니다.
ALLOWED_USER_IDS = [12023760, 1250580892537386] # 예시 ID, 실제 ID로 교체해주세요.

# 임베드 공통 썸네일(외부 이미지 이모지 대용)
EMBED_ICON_URL = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR_xV9-6jLPhxp5TLkKPq1sfTvMADTF4A&s" # 이미지 URL은 적절한 것으로 바꿔주세요.
...
logger = logging.getLogger(__name__) # 'name' 대신 '__name__' 사용
...
class InfoModal(disnake.ui.Modal):
    def __init__(self, serial_code): # __init__으로 수정
        components = [
        ...
        super().__init__( # super().__init__으로 수정
...
# PurchaseCoinDropdown 클래스는 사용되지 않으므로 삭제합니다.

# CoinView 클래스에서 '매입하기'와 '최소송금' 버튼 제거
...
last_embed_update_time = datetime.now() # 임베드 갱신 시간 추적용으로 변경
current_stock = "0"
current_rate = 1350
service_fee_rate = 0.05
update_counter = 0 # 이 변수는 이제 사용되지 않습니다.
    ...
    try:
        ...
        try:
            ...
            admin_embed = disnake.Embed(
                title="✅ 거절 알림 전송 완료",
                description=f"{유저.display_name}님에게 거절 알림을 전송했습니다.",
                color=0x00ff00
            )
...
async def info_lookup(inter, 유저: disnake.Member):
    try:
        ...
        user_id, phone, dob, name, telecom, total_amount, now_amount = row # 변수 이름 중복 방지
    ...
    try:
        ...
        for i, tx in enumerate(transactions, 1):
            if tx.get('txid') or tx.get('api_txid'):
                ...
                txid_text += "\n"
                count += 1
                if len(txid_text) > 3500: # 디스코드 임베드 필드 길이 제한을 고려
                    break
        if count == 0:
...
async def service_embed(inter):
    try:
        await inter.response.defer(ephemeral=True) # Defer early
        ...
        global embed_message, current_stock, current_rate, last_embed_update_time
        ...
        for coin_symbol in supported_coins:
            ...
            if balance > 0:
                ...
                # 현재 환율(current_rate)이 숫자 타입이라고 가정합니다.
                krw_value = balance * price * float(current_rate) # float() 추가하여 형 변환
                total_krw_value += krw_value
        
        # 김치프리미엄 조회
        kimchi_premium = coin.get_kimchi_premium()
        
        embed = disnake.Embed(title="# xdayoungx의 코인대행", color=0xFFFFFF) # 임베드 색상 흰색으로 변경
        try:
        ...
        embed.description = (
            f"▸ 실시간 재고\n"
            f"**__{total_krw_value:,.0f}원__**\n\n"
            f"▸ 실시간 김프\n"
            f"**__{kimchi_premium:+.2f}%__**\n"
        )

        # 다음 갱신까지 남은 시간 계산 (update_embed_task가 300초마다 실행되므로)
        next_update_in_seconds = (last_embed_update_time + timedelta(seconds=300) - datetime.now()).total_seconds()
        next_update_minutes = math.ceil(max(0, next_update_in_seconds) / 60) # 최소 0분, 올림
        
        # 갱신 주기 표시 방식 변경
        embed.add_field(name="\u200b", # 빈 이름 (구분선)
                        value=f"`매 {next_update_minutes}분 뒤 갱신됩니다`", # N분 뒤 갱신됩니다.
                        inline=False)
        embed.set_footer(text="내역 조회는 정보 버튼을 눌려 확인 가능합니다") # 푸터 메시지 추가
        
        view = CoinView() # 변경된 CoinView 사용
        
        # 이전 메시지가 있다면 삭제하고 새 메시지 전송 (메시지 업데이트 방식에 따라 선택)
        # 또는 그냥 새 메시지 전송
...
async def on_modal_submit(interaction):
    try:
                        ...
                        if attempts >= 3:
                            try:
                                await interaction.edit_original_response(
                                    embed=disnake.Embed(title="인증 실패", description="인증 시도가 여러 번 실패했습니다. 처음부터 다시 시도해주세요.", color=0xff0000)
                                )
                            except Exception:
                                pass
                            ...
                            try:
                                await interaction.edit_original_response(
                                    embed=disnake.Embed(title="인증 실패", description="인증번호가 올바르지 않습니다. 다시 입력해주세요.", color=0xff0000),
                                    view=disnake.ui.View(disnake.ui.Button(label="다시 입력", style=disnake.ButtonStyle.red, custom_id="다시입력"))
                                )
                            except Exception:
                                pass
        ...
        elif interaction.custom_id.startswith("amount_modal"):
            await coin.handle_amount_modal(interaction)

    except Exception as e:
        logger.error(f"모달 처리 전체 오류: {e}")
        
@tasks.loop(seconds=300) # 5분 (300초) 마다 갱신
async def update_embed_task():
    global embed_message, current_stock, current_rate, last_embed_update_time, embed_updating, api_update_counter
    ...
    try:
        ...
        # API 호출 빈도를 조절할 필요가 있다면 api_update_counter 활용 (현재는 매 업데이트마다 호출)
        ...
        last_embed_update_time = datetime.now() # 갱신 시간 기록
        ...
        # 갱신 주기 표시 방식 변경
        embed.add_field(name="\u200b", # 빈 이름 (구분선)
                        value=f"`매 {next_update_minutes}분 뒤 재고가 갱신됩니다`", # N분 뒤 갱신됩니다.
                        inline=False)
        embed.set_footer(text="내역 조회는 정보 버튼을 눌려 확인 가능합니다") # 푸터 메시지 추가
        ...
        logger.error(f"업데이트 도중 에러 (HTTP): {e}")
        embed_message = None # 메시지를 찾을 수 없거나 삭제된 경우
        embed_updating = False
    except Exception as e:
        logger.error(f"업데이트 도중 에러: {e}")
        # 오류 발생 시에도 embed_message를 None으로 설정하여 재시도를 막을 수 있음
        embed_message = None 
        embed_updating = False
...
async def on_ready():
    ...
    global current_stock, current_rate, last_embed_update_time
    current_stock = get_stock_amount()
    current_rate = get_exchange_rate()
    last_embed_update_time = datetime.now() # 봇 시작 시점도 갱신 시간으로 기록
...
async def manual_refresh(inter):
    try:
        ...
        current_stock = get_stock_amount()
        current_rate = get_exchange_rate()
        last_embed_update_time = datetime.now() # 수동 갱신 시에도 시간 기록
        
        await inter.response.send_message("임베드를 새로고침했습니다.", ephemeral=True)
        await update_embed_task_function_once() # 갱신 로직 실행
...
async def update_embed_task_function_once():
    ...
    try:
        ...
        embed.add_field(name="\u200b", # 빈 이름 (구분선)
                        value=f"`방금 재고가 수동 갱신되었습니다`", # 수동 갱신 알림
                        inline=False)
        embed.set_footer(text="내역 조회는 정보 버튼을 눌려 확인 가능합니다") # 푸터 메시지 추가
        ...
        if message.guild is None and not message.author.bot:
            ...
            if waiting_amount:
                ...
                class ChargeDecisionView(disnake.ui.View):
                    def __init__(self, user_id, amount): # __init__으로 수정
                    ...
                    async def deny(self, button, inter):
                        ...
                        class RejectReasonModal(disnake.ui.Modal):
                            ...
                            async def callback(modal_inter: disnake.ModalInteraction): # modal_inter 타입 힌트 추가
...
async def send_purchase_log(user_id, coin_name, amount):
    try:
        ...
        if channel:
            ...
            try:
                embed.set_author(name="xdayoungx 코인대행") # 오타 수정
            except Exception:
                pass
            try:
...
if __name__ == "__main__": # name == "main" 대신 __name__ == "__main__"으로 수정
    # 'captcha' 폴더가 없으면 생성 (캡챠 이미지 저장용)
    if not os.path.exists("captcha"):
        os.makedirs("captcha")
    bot.run(TOKEN)
