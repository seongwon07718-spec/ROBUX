...
from utils.ui import err_view, ok_view
...
class LicenseCog(commands.Cog):
    def __init__(self, bot): self.bot = bot; self.db = LicenseDB()
    ...
    async def create(self, i: discord.Interaction, 개수: int, 기간: str):
        ...
        if not 1 <= 개수 <= 50:
            await i.response.send_message(view=err_view("개수는 1~50 사이여야 합니다."), ephemeral=True); return
        await i.response.defer(ephemeral=True)
        keys = self.db.create(개수, LICENSE_PERIODS[기간])
        txt = io.BytesIO("\n".join(keys).encode())
        file = discord.File(txt, filename=f"license_{기간}_{개수}ea.txt")
        ...
        class V(discord.ui.LayoutView):
            container = discord.ui.Container(
                ...
                discord.ui.MediaGallery(
                    discord.MediaGalleryItem(f"attachment://license_{기간}_{개수}ea.txt")
                ),
                accent_color=0x57F287,
            )
        # MediaGallery는 이미지/영상 전용이라 File 컴포넌트로 대체
        class VFile(discord.ui.LayoutView):
            container = discord.ui.Container(
                discord.ui.TextDisplay(
                    f"## 라이센스 생성 완료\n"
                    f"생성 수량 : **{개수}개**\n"
        ...
        await i.followup.send(view=VFile(timeout=None), file=file, ephemeral=True)
    ...
    async def list_lic(self, i: discord.Interaction, 상태: str):
        ...
        file = discord.File(io.BytesIO(("\n".join(lines) or "없음").encode()), filename=f"license_{상태}.txt")
...
async def setup(bot): await bot.add_cog(LicenseCog(bot))
