...
if not GMAIL_USER or not GMAIL_APP_PW:
    print("⚠️  경고: .env에 GMAIL_USER 또는 GMAIL_APP_PW 없음!")
else:
    print(f"✅ Gmail: {GMAIL_USER}")
print("=" * 50)

app = FastAPI(title="OTT 최상급")
        ...
        <tr><td align="center">
          <table width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
            <tr><td style="background:#2563eb;padding:32px;text-align:center;">
              <p style="margin:0;font-size:22px;font-weight:900;color:#fff;">OTT 최상급</p>
            ...
            <tr><td style="padding:36px 32px;">
              ...
              <p style="margin:0 0 28px;font-size:14px;color:#64748b;line-height:1.6;">아래 6자리 인증 코드를 입력해주세요.<br>코드는 <strong>5분간</strong> 유효합니다.</p>
              ...
              <p style="margin:0;font-size:12px;color:#94a3b8;">본인이 요청하지 않았다면 이 이메일을 무시해주세요.</p>
            </td></tr>
...
def serve_html():
    if not HTML_PATH.exists():
        raise HTTPException(status_code=404, detail=f"login.html 없음: {HTML_PATH}")
    return HTMLResponse(content=HTML_PATH.read_text(encoding="utf-8"))


@app.get("/health")
def health():
    return {"status": "ok", "gmail": bool(GMAIL_USER)}


@app.post("/send-code")
def send_code(req: SendCodeRequest):
    if not GMAIL_USER or not GMAIL_APP_PW:
        raise HTTPException(status_code=500, detail=".env에 GMAIL 설정이 없습니다.")
    code = "".join(random.choices(string.digits, k=6))
    code_store[req.email] = {
        "code": code,
