...
if not GMAIL_USER or not GMAIL_APP_PW:
    print("⚠️  경고: .env 파일에 GMAIL_USER 또는 GMAIL_APP_PW가 없습니다!")
        ...
        <tr><td align="center">
          <table width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
            ...
            <tr><td style="padding:36px 32px;">
              ...
              <p style="margin:0;font-size:12px;color:#94a3b8;line-height:1.6;">본인이 요청하지 않았다면 이 이메일을 무시해주세요.</p>
            </td></tr>
...
def serve_html():
    if not HTML_PATH.exists():
        raise HTTPException(
            status_code=404,
            detail=f"login.html 파일을 찾을 수 없습니다. 경로: {HTML_PATH}"
        )
...
@app.get("/health")
def health():
    return {"status": "정상 동작 중", "gmail": bool(GMAIL_USER)}
...
def send_code(req: SendCodeRequest):
    if not GMAIL_USER or not GMAIL_APP_PW:
        raise HTTPException(
            status_code=500,
            detail=".env 파일에 GMAIL_USER와 GMAIL_APP_PW를 설정해주세요."
        )
