import os
import asyncio
from datetime import datetime, timezone
from threading import Thread

import aiohttp
import discord
from discord import app_commands
from discord.ext import commands
from dotenv import load_dotenv

# 봇 내부 HTTP 서버 (server.py → 봇 알림용)
from aiohttp import web

load_dotenv()

DISCORD_TOKEN         = os.environ["DISCORD_TOKEN"]
DISCORD_CLIENT_SECRET = os.environ["DISCORD_CLIENT_SECRET"]
SERVER_URL            = os.environ.get("SERVER_URL", "http://localhost:5000")
GUILD_ID              = os.environ.get("ADMIN_GUILD_ID")
SIGNUP_CHANNEL_ID     = int(os.environ.get("SIGNUP_CHANNEL_ID", 0))
BOT_INTERNAL_PORT     = int(os.environ.get("BOT_INTERNAL_PORT", 6000))

intents = discord.Intents.default()
bot     = commands.Bot(command_prefix="!", intents=intents)
tree    = bot.tree


# ══════════════════════════════════════════════
# 봇 내부 HTTP 서버 — server.py 가 여기로 POST
# ══════════════════════════════════════════════
internal_app = web.Application()

async def handle_signup_notify(req: web.Request):
    """server.py 가 회원가입 시 POST /notify/signup 호출"""
    # 인증
    if req.headers.get("X-Discord-Client-Secret") != DISCORD_CLIENT_SECRET:
        return web.json_response({"ok": False, "message": "인증 실패"}, status=403)

    data         = await req.json()
    email        = data.get("email", "")
    method       = data.get("method", "")
    discord_name = data.get("discord_name", "")

    # 채널에 ui.Container 전송
    asyncio.run_coroutine_threadsafe(
        send_signup_notify(email, method, discord_name),
        bot.loop
    )
    return web.json_response({"ok": True})

internal_app.router.add_post("/notify/signup", handle_signup_notify)


async def run_internal_server():
    runner = web.AppRunner(internal_app)
    await runner.setup()
    site = web.TCPSite(runner, "0.0.0.0", BOT_INTERNAL_PORT)
    await site.start()
    print(f"[봇] 내부 HTTP 서버 포트: {BOT_INTERNAL_PORT}")


# ══════════════════════════════════════════════
# 회원가입 알림 — ui.Container 방식
# ══════════════════════════════════════════════
async def send_signup_notify(email: str, method: str, discord_name: str = ""):
    if not SIGNUP_CHANNEL_ID:
        print("[봇] SIGNUP_CHANNEL_ID 미설정")
        return

    channel = bot.get_channel(SIGNUP_CHANNEL_ID)
    if not channel:
        print(f"[봇] 채널 없음: {SIGNUP_CHANNEL_ID}")
        return

    now     = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
    account = discord_name if discord_name else email

    con = discord.ui.Container(accent_colour=discord.Colour(0x5865F2))

    con.add_item(discord.ui.TextDisplay(
        "### 🎉 새 회원 가입\n"
        "-# SailorMarket 가입 알림"
    ))

    con.add_item(discord.ui.Separator(spacing=discord.SeparatorSpacing.small))

    con.add_item(discord.ui.TextDisplay(
        f"-# - **계정**: {account}\n"
        f"-# - **이메일**: {email if email else '*(Discord 전용)*'}\n"
        f"-# - **가입 방식**: {method}\n"
        f"-# - **가입 시각**: {now}"
    ))

    con.add_item(discord.ui.Separator(spacing=discord.SeparatorSpacing.small))

    btn = discord.ui.Button(
        label="관리자 대시보드",
        style=discord.ButtonStyle.link,
        url="https://sailor-piece.shop/admin/dashboard"
    )
    con.add_item(discord.ui.ActionRow(btn))

    await channel.send(components=[con])
    print(f"[봇] 가입 알림 전송: {account}")


# ══════════════════════════════════════════════
# 관리자 계정 생성 모달
# ══════════════════════════════════════════════
class AdminAccountModal(discord.ui.Modal, title="🔐 관리자 계정 생성"):
    email = discord.ui.TextInput(
        label="이메일",
        placeholder="admin@example.com",
        required=True,
        max_length=100
    )
    password = discord.ui.TextInput(
        label="비밀번호",
        placeholder="8자 이상",
        required=True,
        min_length=8,
        max_length=100
    )
    password_confirm = discord.ui.TextInput(
        label="비밀번호 확인",
        placeholder="비밀번호 재입력",
        required=True,
        min_length=8,
        max_length=100
    )

    async def on_submit(self, interaction: discord.Interaction):
        # ── 비밀번호 불일치 ──
        if self.password.value != self.password_confirm.value:
            con = discord.ui.Container(accent_colour=discord.Colour(0xED4245))
            con.add_item(discord.ui.TextDisplay(
                "### ❌ 비밀번호 불일치\n"
                "-# 비밀번호가 일치하지 않습니다. 다시 시도해주세요."
            ))
            await interaction.response.send_message(components=[con], ephemeral=True)
            return

        await interaction.response.defer(ephemeral=True)

        try:
            async with aiohttp.ClientSession() as sess:
                resp = await sess.post(
                    f"{SERVER_URL}/internal/create_admin",
                    json={
                        "email":    self.email.value.strip(),
                        "password": self.password.value,
                    },
                    headers={"X-Discord-Client-Secret": DISCORD_CLIENT_SECRET},
                    timeout=aiohttp.ClientTimeout(total=10),
                )
                data = await resp.json()

            if resp.status == 200:
                con = discord.ui.Container(accent_colour=discord.Colour(0x57F287))
                con.add_item(discord.ui.TextDisplay(
                    "### ✅ 관리자 계정 생성 완료"
                ))
                con.add_item(discord.ui.Separator(spacing=discord.SeparatorSpacing.small))
                con.add_item(discord.ui.TextDisplay(
                    f"-# - **이메일**: {self.email.value.strip()}\n"
                    f"-# 이제 웹사이트에서 관리자 대시보드에 접근할 수 있습니다."
                ))
                btn = discord.ui.Button(
                    label="대시보드 바로가기",
                    style=discord.ButtonStyle.link,
                    url="https://sailor-piece.shop/admin/dashboard"
                )
                con.add_item(discord.ui.ActionRow(btn))
                await interaction.followup.send(components=[con], ephemeral=True)

            else:
                con = discord.ui.Container(accent_colour=discord.Colour(0xED4245))
                con.add_item(discord.ui.TextDisplay(
                    f"### ❌ 오류 발생\n"
                    f"-# {data.get('message', '알 수 없는 오류')}"
                ))
                await interaction.followup.send(components=[con], ephemeral=True)

        except aiohttp.ClientConnectorError:
            con = discord.ui.Container(accent_colour=discord.Colour(0xED4245))
            con.add_item(discord.ui.TextDisplay(
                "### ❌ 서버 연결 실패\n"
                "-# server.py 가 실행 중인지 확인하세요."
            ))
            await interaction.followup.send(components=[con], ephemeral=True)

        except Exception as e:
            con = discord.ui.Container(accent_colour=discord.Colour(0xED4245))
            con.add_item(discord.ui.TextDisplay(
                f"### ❌ 예외 발생\n-# `{e}`"
            ))
            await interaction.followup.send(components=[con], ephemeral=True)

    async def on_error(self, interaction: discord.Interaction, error: Exception):
        con = discord.ui.Container(accent_colour=discord.Colour(0xED4245))
        con.add_item(discord.ui.TextDisplay(f"### ❌ 오류\n-# {error}"))
        await interaction.response.send_message(components=[con], ephemeral=True)


# ══════════════════════════════════════════════
# 슬래시 명령어
# ══════════════════════════════════════════════
@tree.command(name="관리자_아이디", description="관리자 계정을 생성합니다 (서버 관리자 전용)")
@app_commands.checks.has_permissions(administrator=True)
async def cmd_admin_create(interaction: discord.Interaction):
    await interaction.response.send_modal(AdminAccountModal())

@cmd_admin_create.error
async def cmd_error(interaction: discord.Interaction, error):
    if isinstance(error, app_commands.MissingPermissions):
        con = discord.ui.Container(accent_colour=discord.Colour(0xED4245))
        con.add_item(discord.ui.TextDisplay(
            "### ❌ 권한 없음\n"
            "-# 이 명령어는 서버 관리자만 사용할 수 있습니다."
        ))
        await interaction.response.send_message(components=[con], ephemeral=True)
    else:
        con = discord.ui.Container(accent_colour=discord.Colour(0xED4245))
        con.add_item(discord.ui.TextDisplay(f"### ❌ 오류\n-# {error}"))
        await interaction.response.send_message(components=[con], ephemeral=True)


# ══════════════════════════════════════════════
# 봇 준비
# ══════════════════════════════════════════════
@bot.event
async def on_ready():
    print(f"[봇] {bot.user} 준비 완료")

    # 내부 HTTP 서버 시작
    await run_internal_server()

    if GUILD_ID:
        guild = discord.Object(id=int(GUILD_ID))
        tree.copy_global_to(guild=guild)
        synced = await tree.sync(guild=guild)
    else:
        synced = await tree.sync()

    print(f"[봇] 슬래시 명령어 동기화: {len(synced)}개")
    print(f"[봇] 알림 채널: {SIGNUP_CHANNEL_ID}")


if __name__ == "__main__":
    bot.run(DISCORD_TOKEN)
