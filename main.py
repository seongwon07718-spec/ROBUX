...
class HistSelect(discord.ui.Select):
    def __init__(self, gid, target_id):
        self.gid = gid; self.target_id = target_id
    ...
    async def callback(self, i: discord.Interaction):
        ...
        if self.values[0] == "charge":
            ...
            txt = "## 최근 충전 내역\n" + ("\n".join(f"• {fmt(r['amount'])} | {sm.get(r['status'],r['status'])} | {r['date'][:10]}" for r in recs) if recs else "없음")
        else:
            recs = gdb.recent_purchases(self.target_id)
            txt = "## 최근 구매 내역\n" + ("\n".join(f"• {r['name']} x{r['qty']} | {fmt(r['price'])} | {r['date'][:10]}" for r in recs) if recs else "없음")
...
class UserAdminCog(commands.Cog):
    ...
    async def user_info(self, i: discord.Interaction, 유저: discord.Member):
        ...
        await i.response.send_message(view=UserInfoView(i.guild.id, 유저, ud), ephemeral=True)
    ...
    async def balance_manage(self, i: discord.Interaction, 유저: discord.Member, 금액: int, 여부: str):
        ...
        if 금액 <= 0:
            class Ve(discord.ui.LayoutView):
                c = discord.ui.Container(discord.ui.TextDisplay("금액은 0보다 커야 합니다."), accent_color=0xED4245)
            await i.response.send_message(view=Ve(timeout=None), ephemeral=True); return

        gdb = GuildDB(i.guild.id)
        before = gdb.get_user(유저.id)["balance"]
        if 여부 == "add":
            gdb.add_balance(유저.id, 금액); action = "추가"
        else:
            if before < 금액:
                class Ve(discord.ui.LayoutView):
                ...
                await i.response.send_message(view=Ve(timeout=None), ephemeral=True); return
            gdb.add_balance(유저.id, -금액); action = "차감"
...
async def setup(bot): await bot.add_cog(UserAdminCog(bot))
